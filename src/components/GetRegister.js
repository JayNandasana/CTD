...
export default function GetRegister() {
  ...
  function downlode() {
      ...
      if (btn === "b4") {
        ...
        var b_html = ` <button style="margin-left:auto;margin-right:auto" class="b4">Register</button>`;
      } else if (btn === "b2") {
        b_css = `.b2 { position: relative; outline: none; text-decoration: none; border-radius: 50px; display: flex; align-items: center; justify-content: center; align-items: center; cursor: pointer; height: 45px; width: 140px; opacity: 1; background-color: rgba(255, 255, 255, 0.5); ${FS}; border: 2px solid rgba(0, 0, 0); } .b2 .b2span { ${FS}; color: #000000; font-size: 12px; font-weight: 500; letter-spacing: 0.7px; } .b2:hover { animation: b2rotate 0.7s ease-in-out both; } .b2:hover .b2span { animation: b2storm 0.7s ease-in-out both; animation-delay: 0.06s; } @keyframes b2rotate { 0% { transform: rotate(0deg) translate3d(0, 0, 0); } 25% { transform: rotate(3deg) translate3d(0, 0, 0); } 50% { transform: rotate(-3deg) translate3d(0, 0, 0); } 75% { transform: rotate(1deg) translate3d(0, 0, 0); } 100% { transform: rotate(0deg) translate3d(0, 0, 0); } } @keyframes b2storm { 0% { transform: translate3d(0, 0, 0) translateZ(0); } 25% { transform: translate3d(4px, 0, 0) translateZ(0); } 50% { transform: translate3d(-3px, 0, 0) translateZ(0); } 75% { transform: translate3d(2px, 0, 0) translateZ(0); } 100% { transform: translate3d(0, 0, 0) translateZ(0); } }`;
        b_html = `<button style="margin-left:auto;margin-right:auto" class="b2">Register</button>`;
      } else if (btn === "b3") {
        b_css = `.b3 { padding: 0.6em 2em; border: 1px solid rgb(0, 0, 0); outline: none; color: rgb(0, 0, 0); background: #111; cursor: pointer; position: relative; align-items: center; display: flex; justify-content: center; height: 45px; width: 140px; z-index: 0; border-radius: 30px; ${FS}; } .b3:before { content: ""; background: linear-gradient( 45deg, #ff0000, #ff7300, #fffb00, #48ff00, #00ffd5, #002bff, #7a00ff, #ff00c8, #ff0000 ); position: absolute; top: -2px; left: -2px; background-size: 400%; z-index: -1; filter: blur(3px); width: calc(100% + 5px); height: calc(100% + 5px); animation: glowingb3 20s linear infinite; opacity: 0; transition: opacity 0.3s ease-in-out; border-radius: 20px; } @keyframes glowingb3 { 0% { background-position: 0 0; } 50% { background-position: 400% 0; } 100% { background-position: 0 0; } } .b3:active { color: #000; } .b3:active:after { background: transparent; } .b3:hover:before { opacity: 1; } .b3:after { z-index: -1; content: ""; position: absolute; width: 100%; height: 100%; background: #fffdfd; left: 0; top: 0; border-radius: 30px; }`;
        b_html = `<button style="margin-left:auto;margin-right:auto" class="b3">Register</button>`;
      } else if (btn === "b1") {
        ...
        b_html = `<button style="margin-left:auto;margin-right:auto" class="b1">Register</button>`;
      } else if (btn === "b5") {
        ...
        b_html = `<br><button style="margin-left:auto;margin-right:auto" class="b5">Register</button>`;
      } else if (btn === "b6") {
        ...
        b_html = `<br><button style="margin-left:auto;margin-right:auto" class="b6">Register</button>`;
      } else if (btn === "b7") {
        ...
        b_html = `<br><button style="margin-left:auto;margin-right:auto" class="b7">Register</button>`;
      } else if (btn === "b8") {
        ...
        b_html = `<br><button style="margin-left:auto;margin-right:auto" class="b8">Register</button>`;
      }
      const data = `<!DOCTYPE html> <html lang="en"> <head> <style media="screen"> *, *:before, *:after { padding: 0; margin: 0; box-sizing: border-box; } body { background-color:${background_color}; } .background { width: 430px; height: 520px; position: absolute; transform: translate(-50%, -50%); left: 50%; top: 50%; } .background .shape { height: 200px; width: 200px; position: absolute; border-radius: 50%; } .shape:first-child { background: linear-gradient(#1845ad, #23a2f6); left: -80px; top: -80px; } .shape:last-child { background: linear-gradient(to right, #ff512f, #f09819); right: -80px; bottom: -80px; } form { height: 520px; width: 400px; background-color: rgba(255, 255, 255, 0.13); position: absolute; transform: translate(-50%, -50%); top: 50%; left: 50%; border-radius: 10px; backdrop-filter: blur(10px); border: 2px solid rgba(255, 255, 255, 0.1); box-shadow: 0 0 40px rgba(8, 7, 16, 0.6); padding: 20px 35px; } form * { ${FS}; ${FC}; letter-spacing: 0.5px; outline: none; border: none; }${b_css}; form h3 { font-size: 32px; font-weight: 500; margin-bottom: 25px; text-align: center; ${FS}; } label { display: block; margin-top: 20px; font-size: 16px; font-weight: 500; ${FS}; } input { display: block; height: 45px; width: 100%; background-color: rgba(255, 255, 255, 0.07); border-radius: 3px; padding: 0 10px; margin-top: 2px; font-size: 14px; font-weight: 300; ${FS}; } ::placeholder { ${FC}; opacity: 0.5; } button { margin-top: 10px; width: 100%; background-color: #ffffff; color: #080710; padding: 15px 0; font-size: 18px; font-weight: 600; border-radius: 5px; cursor: pointer; } .social { margin-top: 30px; display: flex; } .social div { background: red; width: 150px; border-radius: 3px; padding: 5px 10px 10px 5px; background-color: rgba(255, 255, 255, 0.27); color: #eaf0fb; text-align: center; } .social div:hover { background-color: rgba(255, 255, 255, 0.47); } .social .fb { margin-left: 25px; } .social i { margin-right: 4px; } </style> </head> <body> <div class="background"> <div class="shape"></div> <div class="shape"></div> </div> <form> <h3 style="text-align: center">Register Here</h3> <label for="name">Full Name</label> <input type="text" placeholder="Full Name" id="username" /> <label for="phone">Phone Number</label> <input type="text" placeholder="Phone Number" id="username" /> <label for="email">Email</label> <input type="text" placeholder="Email" id="username" /> <label for="password">Password</label> <input type="password" placeholder="Password" id="password" /> ${b_html}; </form> </body> </html>`;
      const blob = new Blob([data], { type: "text/plain;charset=utf-8" });
      saveAs(blob, "register.html");
    }
          ...
          <div className="card">
            <div className="row g-0">
              <div className="col-md-4">
                <img src="./sample/register.png" className="img-fluid rounded-start" alt="Sample1"/>
              </div>
              <div className="col-md-8">
                <div className="card-body" style={{ backgroundColor: '#ffffff', overflow: "auto",height:"75vh" }}>
                  <h5 className="card-title mb-4">Register Form</h5>
  
                  <div className="input-group input-group-lg mb-3 mt-3">
                    <select className="form-select" id="font_style">
                  ...
                  />
                    ...
                    <h5 className="mt-3 mb-2">Choose Register Button</h5>
                  ...
                  <div className="text-center">
                    <button  type="submit" onClick={downlode} className="btn btn-light mt-4" style={{backgroundColor: "#00008b",color:"white" }}>Downlode</button>
                  </div>
                </div>
              </div>
            </div>
